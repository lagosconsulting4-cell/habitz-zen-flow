...
export const Compare = ({
  ...
  const renderFirst = firstContent || (firstImage ? (
    <img
      alt="first image"
      src={firstImage}
      className={cn(
  ...
  const renderSecond = secondContent || (secondImage ? (
    <img
      className={cn(
  ...
  return (
    ...
    >
      <AnimatePresence initial={false}>
        ...
        >
          <div className="w-36 h-full [mask-image:radial-gradient(100px_at_left,white,transparent)] absolute top-1/2 -translate-y-1/2 left-0 bg-gradient-to-r from-primary/40 via-transparent to-transparent z-20 opacity-50" />
          ...
          {showHandlebar && (
            <div className="h-8 w-8 rounded-full top-1/2 -translate-y-1/2 bg-primary z-30 -right-4 absolute flex items-center justify-center shadow-lg">
      ...
      {/* Second content (right side - background) */}
      <AnimatePresence initial={false}>
        {renderSecond && (
          <motion.div
            className={cn(
          ...
          >
            {secondContent ? (
              <div className="absolute inset-0 w-full h-full">{secondContent}</div>
            ) : (
              renderSecond
            )}
          </motion.div>
